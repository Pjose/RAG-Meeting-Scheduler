...
import {
  ...
  useUpdateHiMeeting,
  useListPeople,
  useAssignPersonToHiMeeting,
  useRemovePersonFromHiMeeting,
  getGetHiMeetingQueryKey,
  getListHiMeetingsQueryKey,
...
export default function AdminHiMeetingEdit() {
  const [matchEdit, paramsEdit] = useRoute("/admin/hi-meetings/:id");
  const [matchNew] = useRoute("/admin/hi-meetings/new");
  ...
  const { data: meeting, isLoading } = useGetHiMeeting(id, {
    query: { enabled: !!id && !isNew, queryKey: getGetHiMeetingQueryKey(id) },
  });
  ...
  const createMutation = useCreateHiMeeting();
  const updateMutation = useUpdateHiMeeting();
  const assignMutation = useAssignPersonToHiMeeting();
  const removeMutation = useRemovePersonFromHiMeeting();
  ...
  const onSubmit = async (values: FormValues) => {
    ...
    if (isNew) {
      ...
      queryClient.invalidateQueries({ queryKey: getListHiMeetingsQueryKey() });
      toast({ title: "H&I meeting created" });
      setLocation(`/admin/hi-meetings/${created.id}`);
    } else {
      await updateMutation.mutateAsync({ id, data: payload });
      queryClient.invalidateQueries({ queryKey: getGetHiMeetingQueryKey(id) });
      queryClient.invalidateQueries({ queryKey: getListHiMeetingsQueryKey() });
      toast({ title: "H&I meeting saved" });
    }
  };
    ...
    <AdminLayout>
      <div className="max-w-2xl">
        <div className="flex items-center gap-3 mb-6">
          <Button variant="ghost" size="icon" className="h-8 w-8" onClick={() => setLocation("/admin/hi-meetings")}>
            <ArrowLeft size={15} />
          </Button>
          <div>
            <h1 className="text-xl font-semibold font-serif text-foreground">{isNew ? "New H&I Meeting" : "Edit H&I Meeting"}</h1>
            ...
            <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-5">
              <FormField control={form.control} name="name" render={({ field }) => (
                <FormItem>
                  ...
                  <FormControl><Input {...field} placeholder="e.g. County Jail Tuesday Meeting" /></FormControl>
                  <FormMessage />
                </FormItem>
              )} />
              ...
              <div className="grid grid-cols-2 gap-4">
                ...
                <FormField control={form.control} name="language" render={({ field }) => (
                  <FormItem>
                    ...
                    <Select value={field.value} onValueChange={field.onChange}>
                      <FormControl><SelectTrigger><SelectValue placeholder="Language" /></SelectTrigger></FormControl>
              ...
              <FormField control={form.control} name="location" render={({ field }) => (
                <FormItem>
                  ...
                  <FormControl><Input {...field} placeholder="e.g. County Correctional Facility, Rm 12" /></FormControl>
                </FormItem>
              )} />
              ...
              <FormField control={form.control} name="link" render={({ field }) => (
                <FormItem>
                  ...
                  <FormControl><Input {...field} placeholder="https://zoom.us/..." /></FormControl>
                </FormItem>
              )} />
              ...
              <FormField control={form.control} name="literature" render={({ field }) => (
                <FormItem>
                  ...
                  >
                    <FormControl><SelectTrigger><SelectValue placeholder="Select literature" /></SelectTrigger></FormControl>
                    <SelectContent>
              ...
              <FormField control={form.control} name="notes" render={({ field }) => (
                <FormItem>
                  ...
                  <FormControl><Textarea {...field} placeholder="Special notes..." rows={3} /></FormControl>
                </FormItem>
              )} />
            ...
            {meeting.people && meeting.people.length > 0 ? (
              <div className="space-y-2 mb-5">
                {meeting.people.map((person: any) => (
                  <div key={person.id} className="flex items-center gap-3 p-2.5 bg-muted/40 rounded-lg">
        ...
        <AlertDialogContent>
          <AlertDialogHeader>
            ...
            <AlertDialogDescription>
              Remove <span className="font-medium text-foreground">{removePersonTarget?.name}</span> from this H&amp;I meeting's assignments?
            </AlertDialogDescription>
          </AlertDialogHeader>
          <AlertDialogFooter>
