...
function to24h(time: string) {
  return time; // times stored as HH:MM
}
...
function meetingFitsSlot(startTime: string, slot: string) {
  // A meeting belongs in the slot of its starting hour
  const mHour = startTime.slice(0, 2);
  const sHour = slot.slice(0, 2);
  return mHour === sHour;
}
...
export default function PrintSchedule() {
  ...
  // Only keep slots that have at least one meeting
            ...
            <thead>
              <tr>
                ...
                {DAYS.map((d) => (
                  <th
                    key={d}
                    className="border border-border bg-sidebar text-sidebar-foreground print:bg-gray-800 print:text-white px-2 py-2 font-semibold text-center"
                  >
            ...
            <tbody>
              {(activeSlots.length > 0 ? activeSlots : TIME_SLOTS).map((slot) => {
                const hasAny = DAYS.some((d) =>
                  byDay[d].some((m) => meetingFitsSlot(m.startTime, slot))
                );
                return (
                  <tr
                    key={slot}
                    className={hasAny ? "bg-card" : "bg-background print:bg-white"}
                  >
                    ...
                    {DAYS.map((day) => {
                      const meetings = byDay[day].filter((m) =>
                        meetingFitsSlot(m.startTime, slot)
                      );
                      return (
                        <td
                          key={day}
